...
const CALS = join(REPO_ROOT, 'registry', 'cals')
...
export function startBuilder(port = 8799) {
  const server = createServer(async (req, res) => {
    try {
      ...
      if (req.method === 'POST' && url.pathname === '/api/validate') {
        ...
        return send(200, JSON.stringify(lintCal(cal, cartridges)))
      }
      if (req.method === 'POST' && url.pathname === '/api/save') {
        const { name, cal } = JSON.parse(await readBody(req))
        const cartridges = catalogEntries().map((e) => ({ path: e.path, card: e.card }))
        const lint = lintCal(cal, cartridges)
        mkdirSync(CALS, { recursive: true })
        const dest = join(CALS, safeName(name || cal.id) + '.cal.json')
  ...
  server.listen(port, () => {
    ...
    console.log('  Visual CAL/RAC loop builder. Reads platform/catalog, saves to registry/cals/.')
