...
The `registry/` directory is a **git-shareable public registry**: an open, forkable, PR-reviewed way to
publish cartridges, signed agent-team workflows, signed Agent Graphs, and agent-package templates using
nothing but git. Its CI gate re-verifies every signed artifact and refuses to index anything tampered,
invalid, unsigned, or structurally unclean.
...
| Best for | open, forkable, PR-reviewed sharing of cartridges, workflows, Agent Graphs, and templates | registry-grade distribution: tags, digests, replication | browsing, verifying, and trading a live roster |
...
Whichever path an artifact travels, the verifier at the far end recomputes its canonical digest from live
content and checks the Ed25519 DSSE envelope — the transport never becomes part of the trust story.
...
- **Agent Graphs** (`registry/graphs/<id>.agent-graph.json`) — signed `acx.agent-graph/1` team information
  architectures. They declare context stewardship, direction and reporting routes, knowledge movement, and
  where separate loops converge — as descriptions and references, never private knowledge content.
...
Commit the one signed file prepared under `registry/graphs/<id>.agent-graph.json`; keep the generated
private `.key.pem` out of git. Publication validation rejects secret-like metadata and requires every ACX
Workflow reference to be pinned by id, SemVer, and digest. Structural validation resolves every actor,
knowledge, route, expected return, loop, and convergence reference; rejects self-routes and ambiguous
participant bindings; keeps required direction conflict-free; and requires positive graph and convergence
bounds.

The graph does not grant authority or dispatch work. It makes the team's otherwise implicit information
architecture reviewable: who owns context, who directs or reports to whom, and where separate loops meet.
See the animated [Agent Graph guide](../format/agent-graph.md).
...
can verify itself, a workflow, or an Agent Graph; prepare the canonical registry diff; regenerate the
index; run the suite; and draft the PR body. Remote writes remain human-authorized and registry
...
cartridge, workflow, and Agent Graph in the tree and regenerates `registry/index.json` deterministically
...
indexed 1 signed agent graph(s) -> registry/index.json
  product-delivery     v1.0.0      trust=portable actors=3 routes=4 loops=2
```
...
For each `.agent-graph.json` under `registry/graphs/`, it validates the stricter public profile, graph
references and bounds, recomputes the JCS digest, verifies the distinct Agent Graph DSSE/in-toto signature
and identity bindings, and emits only safe discovery metadata.
...
```mermaid
flowchart LR
    A["fork + author<br/>acx export / workflow sign / graph sign"] --> B["PR: add signed artifact"]
...
workflows, and Agent Graphs without opening every artifact:
...
```json
{
  ...
  "agentGraphs": [
    {
      "path": "graphs/product-delivery.agent-graph.json",
      "id": "product-delivery",
      "version": "1.0.0",
      "publisher": "io.github.lboel",
      "trust": "portable",
...
Cartridges are sorted by provable level then name; workflows and Agent Graphs by id then SemVer. The index
is a *convenience*, not a trust root: the authoritative check is always `acx verify` for a cartridge,
`acx workflow verify` for a workflow, or `acx graph verify` for an Agent Graph against the cloned bytes.
