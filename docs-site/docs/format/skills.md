...
!!! example "Reproduce it"
    Every command above is generated by the repo's proof scripts. Run `node --experimental-sqlite scripts/smoke.mjs` (the reference implementation is zero-dependency: Node ≥ 22.5.0's builtin `node:sqlite` + `node:crypto`), then `sqlite3 <file>.acx -Ax rom/skills/`. See the [conformance reference](../reference/conformance.md) and the full [proofs page](../proofs.md).
